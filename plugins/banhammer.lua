...
user = msg.from.id
   if msg.to.type ~= 'pv' then
 if matches[1] == "kick" or matches[1] == "کیک" and is_mod(msg) then
...
tdcli.deleteMessagesFromUser(msg.to.id, matches[2], dl_cb, nil)
 ...
 if matches[1] == "banall" or matches[1] == "جی بن" and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
kick_user(matches[2], msg.to.id)
 ...
 if matches[1] == "unbanall" or matches[1] == "رفع جی بن" and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
     ...
     if not lang then
			...
			if matches[2] == 'silents' or matches[2] == "سایلنت ها" then
